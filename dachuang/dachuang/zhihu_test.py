...
#
# target = browser.find_element_by_xpath('//img[@class="yidun_bg-img"]')
# template = browser.find_element_by_xpath('//img[@class="yidun_jigsaw"]')
# target_link = target.get_attribute('src')
# template_link = template.get_attribute('src')
# target_img = Image.open(BytesIO(requests.get(target_link).content))
# template_img = Image.open(BytesIO(requests.get(template_link).content))
...
#
# distance = match('target.jpg','template.png')
# #zoo = 1.36  # 缩放系数，需要自己调整大小
# zoo = 3  # 缩放系数，需要自己调整大小
# tracks = get_tracks((distance + 7))
#
# slider = browser.find_element_by_class_name("yidun_slider")
# ActionChains(browser).click_and_hold(slider).perform()
#
